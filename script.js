...
function computerChoice() {
  ...
  const weaponLength = weaponArray.length;
  const weaponPicker = Math.floor(Math.random() * weaponLength);
  const computerFinalPick = weaponArray[weaponPicker];
...
function winComputer() {
  ...
  const resetButton = document.querySelector('.game-end-symbol');
  resetButton.addEventListener('click', playNewGame);
  rock.removeEventListener('click', playNewGame);
  paper.removeEventListener('click', playNewGame);
  scissors.removeEventListener('click', playNewGame);
}
